// Quizzes — 10 single-card quizzes with native ad, 7 pts per correct.
...
export default function QuizzesScreen() {
  ...
  const [popup, setPopup] = useState<{ amount: number; correct: number; total: number } | null>(null);
  const interstitial = useInterstitial();
  const adReward = useRewarded(() => submit(1.5));

  const load = async () => {
    const did = await getDeviceId();
    setData(await api.get(`/explore/quizzes?device_id=${encodeURIComponent(did)}`));
  };
  useEffect(() => { load(); }, []);

  const select = (opt: number) => {
    const next = [...answers]; next[idx] = opt; setAnswers(next);
    setTimeout(() => { if (idx + 1 < (data?.questions?.length || 0)) setIdx(idx + 1); }, 200);
  };

  const submit = async (multiplier = 1.0) => {
    try {
      const did = await getDeviceId();
      const r = await api.post("/explore/quizzes/submit", { device_id: did, answers, multiplier });
      await interstitial.show();
      setPopup({ amount: r.reward, correct: r.correct, total: r.total });
      await load();
  ...
  if (data.done) {
    return (
      <SafeAreaView style={styles.container} edges={["top"]}>
        <Header title="Quizzes" onBack={() => router.back()} />
        <View style={styles.donePane}>
          <Feather name="check-circle" size={48} color={colors.success} />
          <Text style={styles.doneTitle}>Quizzes completed today!</Text>
  ...
  return (
    <SafeAreaView style={styles.container} edges={["top"]}>
      ...
      <ScrollView contentContainerStyle={{ padding: spacing.lg }}>
        <Text style={styles.progress}>QUIZ {idx + 1} OF {data.questions.length} · 7 pts each</Text>
          ...
          <View style={{ marginTop: spacing.md, gap: 8 }}>
            {q?.options?.map((opt: string, i: number) => (
              <Pressable
                ...
                onPress={() => select(i)}
              >
                <View style={[styles.bullet, answers[idx] === i && styles.bulletSel]}>
                  <Text style={[styles.bulletText, answers[idx] === i && { color: "#fff" }]}>{String.fromCharCode(65 + i)}</Text>
                </View>
                <Text style={[styles.optText, answers[idx] === i && { color: colors.primaryDark, fontFamily: fonts.heading }]}>{opt}</Text>
              </Pressable>
            ))}
          </View>
          <AdNative testID="quiz-native-ad" />
        </View>

        {finished && (
          <>
            <Pressable testID="quiz-submit-btn" style={styles.submitBtn} onPress={() => submit()}><Text style={styles.submitText}>Submit Quiz</Text></Pressable>
      ...
      <RewardPopup
        ...
        title="Quiz Result"
        subtitle={`${popup?.correct}/${popup?.total} correct`}
        onClaim={() => { setPopup(null); router.back(); }}
      />
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  opt: { flexDirection: "row", alignItems: "center", gap: 12, padding: 14, backgroundColor: colors.surfaceVariant, borderRadius: radius.button, borderWidth: 1, borderColor: "transparent" },
  optSel: { backgroundColor: colors.primaryLight, borderColor: colors.primary },
  bullet: { width: 28, height: 28, borderRadius: 14, borderWidth: 2, borderColor: colors.textTertiary, alignItems: "center", justifyContent: "center" },
  bulletSel: { backgroundColor: colors.primary, borderColor: colors.primary },
  bulletText: { fontFamily: fonts.heading, color: colors.textSecondary, fontSize: 11 },
  optText: { fontFamily: fonts.body, color: colors.textPrimary, fontSize: 14, flex: 1 },
